...
class TestKkKafkaResourceIT {
    ...
    void setup() {
        ...
        TestKkKafkaResource kafkaResource = new TestKkKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/test-kk-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/test-kk-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
